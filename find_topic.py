...
from topic_modeling.utils import batchify, read_data, get_features
...
def main():
    ...
    merge_phrase_pred = []
    for phrase, pred in zip(phrase_list, all_pred):

        merge_phrase_pred.append([phrase, pred])


    with open(ARGS.topic_results + '/phrase_topics.json', 'w', encoding='utf8') as f:

        json.dump(merge_phrase_pred, f)     
